...
class BootBot extends EventEmitter {
  ...
  constructor(options) {
    super();
    if (!options || (options && (!options.verifyToken || !options.appSecret))) {
      throw new Error('You need to specify verifyToken and appSecret');
    }
    this.accessToken = '';
  ...
  getUserProfile(userId) {
    const url = `https://graph.facebook.com/${this.graphApiVersion}/${userId}?fields=first_name,last_name,profile_pic,locale,timezone,gender,email&access_token=${this.accessToken}`;
    return fetch(url)
      .then(res => res.json())
  ...
  _handleMessageEvent(event) {
    console.log('+++ BootBot _handleMessageEvent', event);
    if (this._handleConversationResponse('message', event)) { return; }
    const text = event.message.text;
    const senderId = event.sender.id;
    const recipientId = event.recipient.id;

    if (this.tokens[recipientId]) {
      this.accessToken = this.tokens[recipientId]['longPageAccessToken'];
    }
    let captured = false;
    if (!text) { return; }
